...
export async function getTicketCounters(req: Request<{}, {}, {}, ITicketCounterQueryParams>, res: Response) {
  try {
    ...
    ticketCounters = await TicketCounter.find(filter).sort(sortResult).skip(skip).limit(limit)
    const total = await TicketCounter.find(filter).countDocuments()

    const results: ITicketCounterResults = { results: ticketCounters, page, totalPages: Math.ceil(total / limit), total }
    res.json(results)
  }
  catch(error) {
