...
import {
  ...
  IDesignationBody,
  IDesignationFilter,
  IDesignationQueryParams,
  IDesignationResults,
} from "../@types/IDesignation";
import { IUserRequest } from "../@types/IUser";
import User from "../models/User";
import { Types } from "mongoose";

export async function getDesignations(
  req: Request<{}, {}, {}, IDesignationQueryParams>,
  res: Response
) {
  try {
    ...
    designations = await Designation.find(filter)
      .sort(sortResult)
      .skip(skip)
      .limit(limit);
    const total = await Designation.find(filter).countDocuments();

    const results: IDesignationResults = {
...
export async function updateDesignation(req: IUserRequest, res: Response) {
  try {
    ...
    designation.updatedBy = req.userId
...
export async function removeDesignation(req: Request, res: Response) {
  try {
    ...
    if (deleted) {
      ...
      res
        .status(500)
        .json({ message: "Error [removeDesignation]: Something went wrong." });
    }
  } catch (error) {
