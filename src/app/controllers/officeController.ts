...
export async function getOffices(
  req: Request<{}, {}, {}, IOfficeQueryParams>,
  res: Response
) {
  try {
    ...
    if (noPage) {
      if (includes === "parentOffice") {
        offices = await Office.find(filter)
          .populate("parentOffice")
          .sort(sortResult);
      } else {
...
export async function createOffice(req: IUserRequest, res: Response) {
  try {
    ...
    const office = new Office({
      ...
      parentOffice: body.parentOffice
        ? new Types.ObjectId(body.parentOffice)
        : undefined,
...
export async function updateOffice(req: IUserRequest, res: Response) {
  try {
    ...
    office.parentOffice = body.parentOffice
      ? new Types.ObjectId(body.parentOffice)
      : undefined;
...
export async function removeOffice(req: Request, res: Response) {
  try {
    ...
    if (deleted) {
      ...
      res
        .status(500)
        .json({ message: "Error [removeOffice]: Something went wrong." });
    }
  } catch (error) {
